...
"multivePenal" <-
 function(formula, formula.terminalEvent, formula2, data, Frailty = TRUE, initialize=TRUE, recurrentAG=FALSE,
  cross.validation=FALSE, n.knots, kappa, maxit=350, hazard="Splines", nb.int, print.times=T)
{
...
call <- match.call()
mCall <- match.call()
mCall$formula <- mCall$formula.terminalEvent <- mCall$formula2 <- mCall$Frailty <- mCall$recurrentAG <-
mCall$initialize <- mCall$cross.validation <- mCall$n.knots <- mCall$kappa <- mCall$maxit <- 
...
#AD
		...
		if(length(kappa)==2){
			...
			kappa3 <- kappa[3] 
			indic.Kappa2 <- 0
#			if(cross.validation)stop("The cross validation is not implemented for three strata")
		}
 
	}else{
		if (!(missing(n.knots)) || !(missing(kappa)) || !(missing(cross.validation))){
			stop("When parametric hazard is not 'Splines' function is specified, 'Kappa1', 'n.knots' and 'cross.validations' arguments must be deleted.")
		}
		n.knots <- 0
		kappa1 <- 0
		kappa2 <- 0
...
#		ind.place <- ind.place -1
	...
	if (!missing(formula2)){
		...
		Terms.formula2 <- terms(formula2, special)
		
		ord.formula2 <- attr(Terms.formula, "order")
	
		if (length(ord.formula2) & any(ord.formula2 != 1)){ 
			stop("Interaction terms are not valid for event formula2")
		}
	}
...
#================                 formula2
# 
# 	varformula2 <- model.matrix(formula2,data=Tab$dataM)
# 	
# 	if (!missing(formula2)){
# 		mformula2 <- mCall
# 		mformula2$data <- Tab$dataM
# 		mformula2$formula <- formula2
...
# 			
 #		if (!missing(formula2))newTerms.formula2<-Terms.formula2
...
# Fonction servant a prendre les termes entre "as.factor"
	...
	if(length(vec.factor.formula2) > 0){
		Xformula2 <- model.matrix(formula2, data=Tab$dataM)
		vect.fact.formula2 <- attr(Xformula2,"dimnames")[[2]]
		occurformula2 <- rep(0,length(vec.factor.formula2))
...
#AD:
    ...
    fit$AIC <- ans$LCV[2]
#AD: 
    fit$theta <- ans$b[np - nvar - indic_alpha]^2
    fit$eta <- ans$b[np - nvar - indic_eta]^2
...
#AD:
    ...
    fit$lam3 <- matrix(ans$lam3, nrow = size3, ncol = 3)
    fit$surv3 <- matrix(ans$surv3, nrow = mt13, ncol = 3)
