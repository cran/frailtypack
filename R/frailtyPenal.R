...
#'
#' \deqn{\eta_i\sim \mathcal{N}(0,\sigma^2)}
#'
...
#' \eqn{i^{th}} subject \eqn{(i=1,...,G)}, the joint gamma frailty model
...
#' \right.}
#'
#' where \eqn{\eta_i\sim \mathcal{N}(0,\sigma^2)}
#'
...
#' \right.}
#'
#' where \eqn{v_i\sim \mathcal{N}(0,\sigma^2)}
#'
...
#'
#' \bold{General Joint Frailty model}
#'
#' Fit a general joint frailty model for recurrent
...
#' \deqn{\left\{
#' \begin{array}{ll}
#' r_{ij}(t|u_i,v_i)=u_iv_ir_0(t)\exp(\bold{\beta_1^{'}Z_{ij}(t)})=u_iv_ir_{ij}(t) & \mbox{(Recurrent)} \\
#' \lambda_{i}(t|u_i)=u_i\lambda_0(t)\exp(\bold{\beta_2^{'}Z_{i}(t)}) = u_i\lambda_{i}(t) & \mbox{(Death)}
#' \end{array}
#' \right.}
#'
...
#' \eqn{\omega_{ij}} for the \eqn{k^{th}} individual of the \eqn{j^{th}} subgroup of
#' the \eqn{i^{th}} group is:
#'
#' \deqn{\left\{
#' \begin{array}{ll}
#' \lambda_{ijk}(t|v_i,\omega_{ij})=v_i\omega_{ij}\lambda_0(t)\exp(\bold{\beta^{'}X_{ijk}}) \\
...
#' \omega_{ij}\sim\Gamma\left(\frac{1}{\eta},\frac{1}{\eta}\right)\hspace{0.05cm}i.i.d. \hspace{0.2cm} \bold{E}(\omega_{ij})=1 \hspace{0.2cm} \bold{Var}(\omega_{ij})=\eta
#' \end{array}
#' \right.}
#' }
#'
...
#'
#' Joint nested frailty models allow studying, jointly, survival processes
#' of recurrent and terminal events for hierarchically clustered data, by
#' considering the terminal event as an informative censoring and by
#' including two iid gamma random effects.
#'
#' The joint nested frailty model includes two shared frailty terms, one for
#' the subgroup (\eqn{u_{fi}}) and one for the group (\eqn{\omega_f}) into the hazard
#' functions. This random effects account the heterogeneity in the data,
#' associated with unobserved covariates. The frailty terms act differently
#' for the two rates (\eqn{u_{fi},\omega_f^{\xi}} for the recurrent rate and \eqn{u_{fi}},
#' \eqn{\omega_i} for the terminal event rate). The covariates could be different for
#' the recurrent rate and death rate.
#'
#' For the \eqn{j^{th}} recurrence \eqn{(j=1,...,n_i)} of the \eqn{j^{th}}
#' individual \eqn{(i=1,...,m_f)} of the \eqn{f^{th}} group \eqn{(f=1,...,n)},
#' the joint nested gamma frailty model for recurrent event hazard function
#' \eqn{r_{fij}(.)} and for terminal event hazard function \eqn{\lambda_{fi}} is:
#'
#'  \deqn{\left\{
#' \begin{array}{ll}
#' r_{fij}(t|\omega_f,u_{fi},\bold{X_{fij}})=r_0(t)u_{fi}\omega_f^{\xi}\exp(\bold{\beta^{'}X_{fij}(t)}) & \mbox{(Recurrent)} \\
#' \lambda_{fi}(t|\omega_f,u_{fi},\bold{X_{fij}})=\lambda_0(t)u_{fi}^{\alpha}\omega_f\exp(\bold{\gamma^{'}X_{fi}(t)}) & \mbox{(Death)}
#' \end{array}
#' \right.}
#'
#' where \eqn{r_0}(resp. \eqn{\lambda_0}) is the recurrent (resp. terminal) event
#' baseline hazard function, \eqn{\beta}(resp. \eqn{\gamma}) the regression coefficient
#' vector, \eqn{\bold{X_{fij}(t)}}(resp. \eqn{\bold{X_{fi}(t)}}) the covariates vectors.
#' The random effects \eqn{\bold{\omega_f \sim \Gamma(\frac{1}{\eta},\frac{1}{\eta})}}
#' and \eqn{\bold{u_{fi} \sim \Gamma(\frac{1}{\theta},\frac{1}{\theta})}}.
#'
#'
#' \if{latex}{
#' \bold{Shared Frailty model}
...
#'
#' The joint nested frailty model includes two shared frailty terms, one for
#' the subgroup (\eqn{u_{fi}}) and one for the group (\eqn{\omega_f}) into the
...
#' the two rates (\eqn{u_{fi}}, \eqn{\omega_f^\xi} for the recurrent rate and
#' \eqn{u_{fi}^\alpha, {\omega_i}} for the terminal event rate). The covariates
...
#' \lambda_{fi}(t|\omega_f, u_{fi}, \bold{X_{fi}})=\lambda_0(t)u_{fi}^\alpha \omega_f \exp(\bold{\gamma'} \bold{X_{fi}}) & \mbox{(Death)}
#' \end{array}
#' \right.}
#'
#' where \eqn{r_0(t)}(resp. \eqn{\lambda_0(t)}) is the recurrent (resp.
...
#' vector. The random effects are \eqn{\omega_f \sim \Gamma \left( \frac{1}{\eta}, \frac{1}{\eta}\right)} and
#' \eqn{u_{fi} \sim \Gamma \left( \frac{1}{\theta}, \frac{1}{\theta} \right)}
#' }
#'
...
#' computed by the \eqn{\Delta}-method (Knight & Xekalaki, 2000).
#'
#' Two variance matrices are provided in the output: \code{varH} is the variance
#' matrix of all parameters and \code{varHIH} is the robust estimation of the variance
#' matrix, that takes into account the penalization. When fitting a joint frailty model,
#' \code{varHIH} is used to compute the p-values of all the parameters.
#'
...
#' initialize = TRUE, init.B, init.Theta, init.Alpha, Alpha, init.hazard.weib, init.Ksi,
#' Ksi, init.Eta, LIMparam = 1e-3, LIMlogl = 1e-3, LIMderiv = 1e-3, print.times =
#' TRUE)
...
#' It can be chosen among 5, 7, 9, 12, 15, 20, 32 and 50. The default is 20 if hazard = "Splines", 32 otherwise.
#' @param nb.gl Number of nodes for the Gaussian-Laguerre quadrature.
#' It can be chosen among 20, 32 and 50. The default is 20 if hazard = "Splines", 32 otherwise.
...
#' @param init.hazard.weib Only if a weibull model is used. A vector of initial
#' and positive values for the hazard function parameters. This vector is of size 2
#' for a shared frailty model, is of size 2 * n.strat for a stratified shared frailty
#' model, is of size 2 for nested frailty model, is of size 4 for a stratified nested
#' frailty model  (the order is: shapeStrat1, scaleStrat1, shapeStrat2, scaleStrat2…),
#' is of size 4 for Joint frailty model (the order is shapeR, scaleR, shapeD, scaleD),
#' is of size 6 in case of a joint stratified frailty model as stratification in this
#' case is only possible for recurrent event (order in case of a joint stratified frailty
#'  model is (shapeRStrat1, scaleRStrat1,shapeRStrat2, scaleRStrat2, shapeD, scaleD) .
#'  Default is 0.1 for each for shared frailty model and stratified shared frailty model,
#'  0.8 for each for nested frailty model and stratified nested frailty model, 0.5 for each
#'  for  Joint frailty model and joint stratified frailty model.
...
#' \item{lam}{Matrix (dim=3) of hazard estimates and confidence bands.}
#' \item{surv}{Matrix (dim=3) of baseline survival estimates and confidence
...
#' event.} \item{lamR}{Matrix (dim=3) of hazard estimates and confidence bands
#' for recurrent event.} \item{lamD}{the same value as lamR for the terminal
#' event.} \item{survR}{Matrix (dim=3) of baseline survival estimates and
...
#' \item{eta}{variance of the subcluster effect \eqn{(\bold{Var}(\omega_{ij}))}}
...
#'
#' frailtyPenal(
#'   Surv(t.start, t.stop, event) ~ cluster(id) + sex + dukes +
#'     charlson,
#'   data = readmission, n.knots = 6, kappa = 5000, recurrentAG = TRUE,
#'   cross.validation = TRUE
#' )
#'
...
#'   Surv(time, event) ~ cluster(id) + sex + dukes + charlson +
#'     terminal(death),
...
#' #-- log-normal distribution
#'
#' modJoint.log <- frailtyPenal(
#'   Surv(t.start, t.stop, event) ~ cluster(id) + sex
...
#' data(dataNCC)
#' modJoint.ncc <- frailtyPenal(
#'   Surv(t.start, t.stop, event) ~ cluster(id) + cov1
#'     + cov2 + terminal(death) + wts(ncc.wts),
#'   formula.terminalEvent = ~ cov1 + cov2,
...
#'
#' joi.clus.gap <- frailtyPenal(
#'   Surv(time, event) ~ cluster(group) +
...
#'
#' data(readmission)
#' modJoint.general <- frailtyPenal(
#'   Surv(time, event) ~ cluster(id) + dukes +
...
#' data(dataNested)
#' modClu <- frailtyPenal(
#'   Surv(t1, t2, event) ~ cluster(group) +
#'     subcluster(subgroup) + cov1 + cov2,
#'   data = dataNested,
#'   n.knots = 8, kappa = 50000
#' )
#'
#' modClu.str <- frailtyPenal(
#'   Surv(t1, t2, event) ~ cluster(group) +
#'     subcluster(subgroup) + cov1 + strata(cov2),
#'   data = dataNested,
...
#'
#' modJointNested_Splines <- frailtyPenal(
...
#' )
#'
#' modJointNested_Weib <- frailtyPenal(
#'   Surv(t.start, t.stop, event) ~ subcluster(id)
#'     + cluster(group) + dukes + terminal(death),
#'   formula.terminalEvent = ~dukes,
#'   hazard = ("Weibull"), data = readmissionNested, recurrentAG = TRUE, initialize = FALSE
#' )
#'
#' JoiNesGapSpline <- frailtyPenal(
...
"frailtyPenal" <-
  function(formula, formula.terminalEvent, data, recurrentAG = FALSE, cross.validation = FALSE, jointGeneral, n.knots, kappa, maxit = 300,
           hazard = "Splines", nb.int, RandDist = "Gamma", nb.gh, nb.gl, betaknots = 1, betaorder = 3, initialize = TRUE, init.B, init.Theta, init.Alpha, Alpha, init.hazard.weib, init.Ksi, Ksi, init.Eta,
    ...
    if (!(RandDist %in% c("Gamma", "LogN"))) {
      stop("Only 'Gamma' and 'LogN' distributions for frailties are allowed")
    }
    logNormal <- switch(RandDist,
      "Gamma" = 0,
      "LogN" = 1
    )
    ...
    if (!(length(hazard) %in% c(1, 2))) {
      stop("Please check and revise the hazard argument according to the format specified in the help.")
    }
    ...
    if (!(nb.gh %in% c(5, 7, 9, 12, 15, 20, 32, 50))) stop("nb.gh must be chosen among 5,7,9,12,15,20,32 and 50")
    if (missing(nb.gl)) {
      if (typeof == 0) {
        nb.gl <- 20
      } else {
    ...
    if (!(nb.gl %in% c(20, 32, 50))) stop("nb.gl must be chosen among 20, 32 and 50")
    ...
    m$formula.terminalEvent <- m$n.knots <- m$recurrentAG <- m$cross.validation <- m$jointGeneral <- m$kappa <- m$maxit <- m$hazard <- m$nb.int <- m$RandDist <- m$betaorder <- m$betaknots <- m$init.B <- m$LIMparam <- m$LIMlogl <- m$LIMderiv <- m$print.times <- m$init.hazard.weib <- m$init.Theta <- m$init.Alpha <- m$Alpha <- m$init.Ksi <- m$Ksi <- m$init.Eta <- m$Eta <- m$initialize <- m$nb.gh <- m$nb.gl <- m$... <- NULL
    ...
    if (length(subcluster) && Frailty == TRUE) {
      ...
      for (i in 2:length(subcluster)) {
        if (subcluster[i] == curr) {
          subcluster[i] <- subcluster[i - 1]
        } else {
          curr <- subcluster[i]
    ...
    if (classofY == "SurvIC") {
      intcens <- TRUE
    } # booleen censure par intervalle
    else {
      intcens <- FALSE
    }
    ...
    if (length(vec.factor) > 0) {
      ...
      interaction <- as.vector(apply(matrix(vect.fact, nrow = length(vect.fact)), MARGIN = 1, FUN = function(x) {
        length(grep(":", unlist(strsplit(x, split = ""))))
      }))
      ...
      for (i in 1:length(vec.factor)) {
        if (length(grep(":", unlist(strsplit(vec.factor[i], split = "")))) > 0) {
          ...
          if (length(vect.fact[-which.interaction]) > 0) {
            occur[i] <- length(grep(vec.factor[i], vect.fact[-which.interaction]))
          } else {
    ...
    if (length(strats)) {
      ...
      if (length(subcluster)) {
        if (uni.strat > 2) {
          if (joint) {
            stop("maximum number of strata for nested frailty joint model is 2")
          } else {
            stop("maximum number of strata for nested frailty joint model is 2")
          }
        }
    ...
    if (!joint & !length(subcluster)) {
      ...
      if (uni.strat == 1) {
        if (!missing(init.hazard.weib)) {
          ...
          if (length(init.hazard.weib) != as.integer(uni.strat) * 2) stop("init.hazard.weib must be a vector of length 2 for Shared Frailty model")
          ...
          Beta[1:(as.integer(uni.strat) * 2)] <- c(sqrt(init.hazard.weib))
        }
      }
      ...
      if (uni.strat > 1) {
        if (!missing(init.hazard.weib)) {
          if (typeof != 2) stop("init.hazard.weib is only available for Weibull hazard")
          if (length(init.hazard.weib) != as.integer(uni.strat) * 2) stop("init.hazard.weib must be a vector of length 2*n.strat for stratified Shared Frailty model")
          if (!all(sapply(init.hazard.weib, function(x) is.numeric(x) && x >= 0))) stop("init.hazard.weib must be a vector of positive real numbers")
          Beta[1:(as.integer(uni.strat) * 2)] <- c(sqrt(init.hazard.weib))
        }
      }
      ...
      if (Frailty) {
        ...
        if (logNormal == 0) {
          fit$theta <- (ans[[20]][np - nvar - npbetatps])^2
        } else {
          fit$sigma2 <- (ans[[20]][np - nvar - npbetatps])^2
        }
      }
      if (!Frailty) {
        if (logNormal == 0) {
      ...
      if (Frailty) {
        ...
        if (logNormal == 0) {
          fit$theta_p.value <- 1 - pnorm(fit$theta / sqrt(fit$varTheta[1]))
        } else {
          fit$sigma2_p.value <- 1 - pnorm(fit$sigma2 / sqrt(fit$varTheta[1]))
        }
      }
      ...
      fit$lam <- if (typeof == 1) {
        array(ans[[26]][seq(1, length(ans[[26]]), 3)], dim = c(nb.int, 3, uni.strat))
      } else {
        array(ans[[26]], dim = c(size1, 3, uni.strat))
      } # Le lam s'ecrit differemment selon la fonction de hasard (Piecewise selon le nombre d'intervalles specifies, Weibull et Splines selon size1 = 100)
    ...
    if (joint & !length(subcluster)) {
      ...
      if (missing(formula.terminalEvent)) {
        m2$n.knots <- m2$recurrentAG <- m2$cross.validation <- m2$kappa <- m2$maxit <- m2$hazard <- m2$nb.int <- m2$RandDist <- m2$betaorder <- m2$betaknots <- m2$init.B <- m2$init.hazard.weib <- m2$LIMparam <- m2$LIMlogl <- m2$LIMderiv <- m2$print.times <- m2$init.Theta <- m2$init.Alpha <- m2$Alpha <- m2$init.Ksi <- m2$Ksi <- m2$init.Eta <- m2$Eta <- m2$initialize <- m2$nb.gh <- m2$nb.gl <- m2$... <- NULL
      } else {
        m2$formula.terminalEvent <- m2$n.knots <- m2$recurrentAG <- m2$cross.validation <- m2$jointGeneral <- m2$kappa <- m2$maxit <- m2$hazard <- m2$nb.int <- m2$RandDist <- m2$betaorder <- m2$betaknots <- m2$init.B <- m2$init.hazard.weib <- m2$LIMparam <- m2$LIMlogl <- m2$LIMderiv <- m2$print.times <- m2$init.Theta <- m2$init.Alpha <- m2$Alpha <- m2$init.Ksi <- m2$Ksi <- m2$init.Eta <- m2$Eta <- m2$initialize <- m2$nb.gh <- m2$nb.gl <- m2$... <- NULL
      }
      ...
      if (!missing(formula.terminalEvent)) {
        ...
        if (length(vec.factordc) > 0) {
          ...
          #    }
          #  }
          interactiondc <- as.vector(apply(matrix(vect.factdc, nrow = length(vect.factdc)), MARGIN = 1, FUN = function(x) {
            length(grep(":", unlist(strsplit(x, split = ""))))
          }))
      ...
      if (!missing(Alpha)) { # new : joint more flexible alpha = 0
        if (Alpha == "None") {
          ...
          stop("Alpha can only take 'None' as a value in this version of frailtypack package")
        }
      }
      nst <- uni.strat # 2
      ...
      if (uni.strat == 1) {
        if (!missing(init.hazard.weib)) {
          if (typeof != 2) stop("init.hazard.weib is only available for Weibull hazard")
          if (length(init.hazard.weib) != (2 + as.integer(uni.strat) * 2)) stop("init.hazard.weib must be a vector of length 4 for Joint Frailty model")
          if (!all(sapply(init.hazard.weib, function(x) is.numeric(x) && x >= 0))) stop("init.hazard.weib must be a vector of positive real numbers")
          Beta[1:(2 + as.integer(uni.strat) * 2)] <- c(sqrt(init.hazard.weib))
        }
      }
      ...
      if (uni.strat > 1) {
        if (!missing(init.hazard.weib)) {
          if (typeof != 2) stop("init.hazard.weib is only available for Weibull hazard")
          if (length(init.hazard.weib) != (2 + as.integer(uni.strat) * 2)) stop("init.hazard.weib must be a vector of length 2*(n.strat+1) for stratified Joint Frailty model")
          if (!all(sapply(init.hazard.weib, function(x) is.numeric(x) && x >= 0))) stop("init.hazard.weib must be a vector of positive real numbers")
          Beta[1:(2 + as.integer(uni.strat) * 2)] <- c(sqrt(init.hazard.weib))
        }
      }
      ...
      # AD:
      if (logNormal == 0) {
        fit$theta <- ans$b[np - nvar - npbetatps - indic_alpha]^2
      } else {
        fit$sigma2 <- ans$b[np - nvar - npbetatps - indic_alpha]^2
      }
      ...
      if (indic_alpha == 1) fit$alpha_p.value <- 1 - pchisq((fit$alpha / sqrt(diag(fit$varHIH))[2])^2, 1)

      if (logNormal == 0) {
        seHIH.frail <- sqrt(((2 * (fit$theta^0.5))^2) * diag(fit$varHIH)[1]) # delta method
        fit$theta_p.value <- 1 - pnorm(fit$theta / seHIH.frail)
      } else {
        seHIH.frail <- sqrt(((2 * (fit$sigma2^0.5))^2) * diag(fit$varHIH)[1]) # delta method
        fit$sigma2_p.value <- 1 - pnorm(fit$sigma2 / seHIH.frail)
      }
      ...
      if (indic_alpha == 1) {
        ...
        noms <- c("theta", factor.names(colnames(X)), factor.names(colnames(X2)))
      }
      if (length(grep(":", noms)) > 0) noms <- factor.names(noms)
      if (timedep == 1) { # on enleve les variances des parametres des B-splines
        while (length(grep("timedep", noms)) != 0) {
      ...
      fit$lamR <- if (typeof == 1) {
        array(ans$lamR[seq(1, length(ans$lamR), 3)], dim = c(nb.int[1], 3, uni.strat))
      } else {
        array(ans$lamR, dim = c(size1, 3, uni.strat))
      }
      fit$xSuR <- matrix(ans$xSuR, nrow = 100, ncol = uni.strat)
      fit$survR <- array(ans$survR, dim = c(mt11, 3, uni.strat))

      fit$xD <- ans$xD
      fit$lamD <- if (typeof == 1) {
        matrix(ans$lamD[seq(1, length(ans$lamD), 3)], nrow = nb.int[2], ncol = 3)
      } else {
        matrix(ans$lamD, nrow = size2, ncol = 3)
      }
      fit$xSuD <- ans$xSuD
      ...
      if (Frailty) {
        if (any(MartinGale[, 1] == 0)) {
          ...
          fit$linear.pred <- ans$linear.pred[order(ordre)]
          if (joint.clust == 0) {
            fit$lineardeath.pred <- ans$linearpredG
          } else {
      ...
      if ((length(vec.factor) > 0) & (timedep == 0)) {
        ...
        if (indic_alpha == 1) {
          VarBeta <- diag(diag(fit$varHIH)[-c(1, 2)])
        } else {
          VarBeta <- diag(diag(fit$varHIH)[-c(1)])
        }
        nfactor <- length(vec.factor)
        p.wald <- rep(0, nfactor)
      ...
      if (!missing(formula.terminalEvent)) {
        if ((length(vec.factordc) > 0) & (timedep == 0)) {
          ...
          if (indic_alpha == 1) {
            VarBeta <- fit$varHIH[3:dim(fit$varHIH)[1], 3:dim(fit$varHIH)[2]]
          } else {
            VarBeta <- fit$varHIH[2:dim(fit$varHIH)[1], 2:dim(fit$varHIH)[2]]
          }
      ...
      if (!is.null(fit$coef)) {
        if (indic_alpha == 1 || fit$joint.clust == 2) {
          seHIH <- sqrt(diag(fit$varHIH))[-c(1, 2)]
        } else {
          seHIH <- sqrt(diag(fit$varHIH))[-1]
        }
        fit$beta_p.value <- 1 - pchisq((fit$coef / seHIH)^2, 1)
      }
    ...
    if (length(subcluster) & !joint) {
      ...
      if (uni.strat > 1) {
        if (!missing(init.hazard.weib)) {
          if (typeof != 2) stop("init.hazard.weib is only available for Weibull hazard")
          if (length(init.hazard.weib) != as.integer(uni.strat) * 2) stop("init.hazard.weib must be a vector of length 2*n.strat for stratified Nested Frailty model")
          if (!all(sapply(init.hazard.weib, function(x) is.numeric(x) && x >= 0))) stop("init.hazard.weib must be a vector of positive real numbers")
          Beta[1:(as.integer(uni.strat) * 2)] <- c(sqrt(init.hazard.weib))
        }
      }
      ...
      if (uni.strat == 1) {
        if (!missing(init.hazard.weib)) {
          if (typeof != 2) stop("init.hazard.weib is only available for Weibull hazard")
          if (length(init.hazard.weib) != as.integer(uni.strat) * 2) stop("init.hazard.weib must be a vector of length 2 for Nested Frailty model")
          if (!all(sapply(init.hazard.weib, function(x) is.numeric(x) && x >= 0))) stop("init.hazard.weib must be a vector of positive real numbers")
          Beta[1:(as.integer(uni.strat) * 2)] <- c(sqrt(init.hazard.weib))
        }
      }
      ...
      fit$lam <- if (typeof == 1) {
        array(c(ans$lam[seq(1, length(ans$lam), 3)], ans$lam2[seq(1, length(ans$lam2), 3)]), dim = c(nb.int, 3, 2))
      } else {
        array(c(ans$lam, ans$lam2), dim = c(size1, 3, 2))
      }
    ...
    if (length(subcluster) & joint) {
      ...
      if (classofY == "Surv") {
        ...
        stop("Interval-censored data are not allowed for joint nested model.")
      }
      ...
      if (!is.null(ncol(vardc.temp))) {
        ...
        vardc <- aggregate(vardc.temp, by = list(sort(subcluster)), FUN = function(x) x[length(x)])[, 2]
      }
      ...
      # =======================================>
      # ======= Construction du vecteur des indicatrices
      ...
      if (!missing(Ksi)) {
        if (Ksi == "None") {
          ...
          stop("Ksi can only take 'None' as a value in this version of frailtypack package")
        }
      }
      ...
      nst <- uni.strat
      if (typeof == 1) {
        stop("!Warning! Piecewise baseline hazard function is not yet allowed for joint nested frailty model.")
      } else {
        indic.nb.int1 <- 0
        indic.nb.int2 <- 0
      }
      np <- switch(as.character(typeof),
        ...
        "2" = (2 * (nst + 1) + nvar + effet + indic_alpha + indic_xi)
      ) # weibull
      ...
      if (!missing(init.hazard.weib)) {
        if (typeof != 2) stop("init.hazard.weib is only available for Weibull hazard")
        if (length(init.hazard.weib) != 4) stop("init.hazard.weib must be a vector of length 4 for Nested Joint Frailty model")
        if (!all(sapply(init.hazard.weib, function(x) is.numeric(x) && x >= 0))) stop("init.hazard.weib must be a vector of positive real numbers")
        Beta[1:4] <- c(sqrt(init.hazard.weib))
      }
      # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      if (length(vec.factor) > 0) {
        ...
        if (indic_alpha == 1) {
          ...
          if (indic_xi == 1) {
            VarBeta <- fit$varH[5:dim(fit$varH)[1], 5:dim(fit$varH)[2]]
          } else {
            VarBeta <- fit$varH[4:dim(fit$varH)[1], 4:dim(fit$varH)[2]]
          }
        } else {
      ...
      if (length(vec.factordc) > 0) {
        ...
        if (indic_alpha == 1) {
          if (indic_xi == 1) {
            VarBeta <- diag(diag(fit$varH)[-c(1, 2, 3, 4)])
          } else {
            VarBeta <- diag(diag(fit$varH)[-c(1, 2, 3)])
          }
        } else {
          if (indic_xi == 1) {
            VarBeta <- diag(diag(fit$varH)[-c(1, 2, 3)])
      ...
      if (!is.null(fit$coef)) {
        if (indic_alpha == 1 & indic_xi == 1) {
          seH <- sqrt(diag(fit$varH))[-c(1:4)]
        } else if (indic_alpha == 0 | indic_xi == 0) {
          seH <- sqrt(diag(fit$varH))[-c(1:3)]
