...
"print.multivePenal" <- function (x, digits = max(options()$digits - 4, 6), ...) 
{
...
#  		}
	...
	if (!is.null(x$fail)) {
		cat(" multivePenal failed.", x$fail, "\n")
		return()
	}
...
#AD
	if (x$istop == 1){
		if (!is.null(coef)){ 
			...
			cat("\n")
			cat("  Multivariate joint gaussian frailty model for two survival outcomes (potentially recurrent) and a terminal event","\n")
			if (x$typeof == 0){
...
#AD:  
		...
		if (x$typeof == 0){
			cat("   theta (variance of Recurrent 1):", theta, "(SE (H):",
				seH.theta, ")", "(SE (HIH):", seHIH.theta, ")", "\n")
			cat("   eta (variance of Recurrent 2):", eta, "(SE (H):",
				seH.eta, ")", "(SE (HIH):", seHIH.eta, ")", "\n")
...
#AD:
		...
		cat("   n recurrent events of type 1=", x$n.event, " n groups=", x$groups)
		cat("\n")
		...
		if (!is.null(coef)){ 
			cat("\n")
			cat("  Multive joint gamma frailty model for recurrent and a terminal event processes","\n")
			if (x$typeof == 0){
				cat("  using a Penalized Likelihood on the hazard function","\n")
			}else{
				cat("  using a Parametrical approach for the hazard function","\n")
