prediction <- function(fit, data, data.Longi, t, window, event = "Both", group, MC.sample=0){
	...
	if (class(fit) == "jointPenal"){
		...
		if (!fit$intcens){
			if ((event.type == 1) || (event.type == 2)){
				...
				colnames(out$pred1) <- paste("time=", timeAll)
				...
				colnames(out$pred3) <- paste("time=", timeAll)
			}
			
			if ((event.type == 1) || (event.type == 3)){
				out$pred1_rec <- matrix(ans$pred1_rec,nrow=npred,ncol=ntimeAll)
				rownames(out$pred1_rec) <- paste("ind",out$group)
				#colnames(out$pred1_rec) <- c("times",rep(" ",ntimeAll-1))
				colnames(out$pred1_rec) <- paste("time=", timeAll)	
			}
		}
		if ((event.type == 1) || (event.type == 2)){
			...
			colnames(out$pred2) <- paste("time=", timeAll)
		}
		...
		if (ICproba){
			if (!fit$intcens){
				...
				if ((event.type == 1) || (event.type == 3)){					
					...
					rownames(out$predlow1_rec) <- paste("ind",out$group)
					#colnames(out$predlow1_rec) <- c("times",rep(" ",ntimeAll-1))
					colnames(out$predlow1_rec) <- paste("time=", timeAll)
					rownames(out$predhigh1_rec) <- paste("ind",out$group)
					#colnames(out$predhigh1_rec) <- c("times",rep(" ",ntimeAll-1))	
					colnames(out$predhigh1_rec) <- paste("time=", timeAll)	
				}
			}
			if ((event.type == 1) || (event.type == 2)){
			...
			survival.mc <- function(t,ObjFrailty,para1,para2){ # dans les trois cas para1 et para2 seront traites differemment
				if (ObjFrailty$typeof == 0){ # splines
					...
					out <- .Fortran("survival",
						...
						lam=as.double(c(0,0)),
						as.integer(nst),
						PACKAGE = "frailtypack") # lam ajoute suite aux modif de survival
