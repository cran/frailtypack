...
subroutine joint_longi(VectNsujet,ngnzag,k0,tt00,tt10,ic0,groupe0&
    ...
    use donnees,only:MC1,MC2,QMC1000_1,QMC1000_2,QMC500_1,QMC500_2!,!QMC5000_1,QMC5000_2,
    ! add Monte-carlo
...
subroutine QMC_JointModels(ss,func2,ndim)
  ...
  use comon, only:nb1,nodes_number,ut
  use donnees_indiv
...
subroutine dummyMCOMP(int,numpatOMP,&
  ...
  use comon,only:method_GH,invBi_cholDet,nea,typeJoint,nb1,maxmesy,ut!nby,auxig,typeof,nb1,nea
  !use donnees_indiv,only : frailpol,numpat
  !!$ use OMP_LIB 
  Implicit none
...
SUBROUTINE gauherJ21OMP(ss,choix,nnodes,numpat,nmescur,ut2cur,&
  ...
  integer,intent(in)::choix,nnodes
  double precision::auxfunca,funcG2 !func8J,func9J,func10J,func11J, funcTP4J, add TwoPart
  external::funcG2 ! add TwoPart
  integer::j
  double precision,dimension(nnodes):: xx1,ww1
...
double precision function survdcCMOMP(tps,i,it_curOMP,bh,np,frail)
  ...
  use residusM,only:Rdc_res
  use var_surrogate,only:mediation
  use var_mediation,only:nuzm,nuzt,treat_ind,nparammed,betazm,betazt
  double precision, dimension(1,nva3)::x2curG
  double precision, dimension(1,nb1)::z1curG
  !    double precision, dimension(1,nb1)::z1YcurG
  double precision, dimension(1,nb1)::z1BcurG
