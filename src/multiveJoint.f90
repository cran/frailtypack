...
!frailtyEstimates=c(frailtypred,frailtypred2,frailtyvar,frailtyvar2,frailtyCorr)
	...
	linearpred,linearpreddc,linearpredM,ziOut1,ziOut2,ziOut3,time,timedc,timeM)
...
!predictor
	...
	double precision,dimension(nz0(2)+6)::ziOut2	
	double precision,dimension(nz0(3)+6)::ziOut3	
!-------- Parametres shared
...
!		write(*,*)'ICI 0'
		...
		timedepMul,nbinnerknots0Mul,qorder0Mul,filtretps0Mul,BetaTpsMatMul)
...
!		write(*,*)''	
		...
		if(typeof == 0) then		
			if(irep.eq.0) then
				kappaCV(3)=k0s(1) ! kappa branche m̩ta
			else
				kappaCV(3)=kappa1
			end if
...
!		write(*,*)''
		if(typeof == 0) then	
			if(irep.eq.0) then
				kappaCV(2)=k0s(1) ! kappa branche m̩ta
			else
				kappaCV(2)=kappa1
			endif
