...
!========================          FUNCPAJ_SPLINES  - log, single likelihood contrib       ====================
    double precision function funcpajsplines_logindiv(b,np,id,thi,jd,thj,k0,index)
...
!AD:end
    ...
    double precision::int,res2dc,integrale3
...
!ccccccccccccccccccccccccccccccccccccccccc
    ...
    if(g(i).eq.index)then
        ...
        if (c(i).eq.1) then
            res2(index) = res2(index)+dlog(dut1T(nt1(i),stra(i))*vet)
        endif
        if ((res2(index).ne.res2(index)).or.(abs(res2(index)).ge. 1.d30)) then
            funcpajsplines_logindiv=-1.d9
            goto 123
        end if

!     nouvelle version
        res1(index) = res1(index) + ut1T(nt1(i),stra(i))*vet
        if ((res1(index).ne.res1(index)).or.(abs(res1(index)).ge. 1.d30)) then
            funcpajsplines_logindiv=-1.d9
            goto 123
        end if

!     modification pour nouvelle vraisemblance / troncature:
        res3(index) = res3(index) + ut1T(nt0(i),stra(i))*vet
        if ((res3(index).ne.res3(index)).or.(abs(res3(index)).ge. 1.d30)) then
            funcpajsplines_logindiv=-1.d9
            goto 123
        end if
      endif  
...
! dans Joint ng=nb individus
        if(nva2.gt.0)then
            vet2 = 0.d0
            do j=1,nva2
                vet2 = vet2 + bh(np-nva2+j)*dble(vedc(index,j))
            end do
            vet2 = dexp(vet2)
        else
            vet2 = 1.d0
        ...
        if(cdc(index).eq.1)then
            res2dc = dlog(dut2(nt1dc(index))*vet2)
            if ((res2dc.ne.res2dc).or.(abs(res2dc).ge. 1.d30)) then
                funcpajsplines_logindiv=-1.d9
                goto 123
            end if
        endif
...
! pour le calcul des integrales / pour la survie, pas les donnees recurrentes:
        aux1(index)=ut2(nt1dc(index))*vet2
        aux2(index)=aux2(index)+ut2(nt0(index))*vet2 !vraie troncature

        if ((aux1(index).ne.aux1(index)).or.(abs(aux1(index)).ge. 1.d30)) then
            funcpajsplines_logindiv=-1.d9
            goto 123
        end if
        if ((aux2(index).ne.aux2(index)).or.(abs(aux2(index)).ge. 1.d30)) then
            funcpajsplines_logindiv=-1.d9
            goto 123
        end if
...
!---------- calcul de la penalisation -------------------
    ...
    if ((res.ne.res).or.(abs(res).ge. 1.d30)) then
        ...
        funcpajsplines_logindiv = resnonpen
        !do k=1,ng
        !    Rrec(k)=res1(k)
        !    Nrec(k)=nig(k)
        !    Rdc(k)=aux1(k)
...
123     continue
    ...
    end function funcpajsplines_logindiv
