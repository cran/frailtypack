...
!//// fin NEW AMADOU-vvv
!AD:end
!    write(*,*)'ut1',(i,ut1(i),dut1(i),i=1,10)
!    pause    
!-------------------------------------------------------
...
!      write(*,*)'*** funcpaMultivSplines vet',vet,ve(i,j),i,j
            end do
            vet = dexp(vet)
        else
            vet=1.d0
...
!    write(*,*)'k',k,res2dc(k),aux1(k),nt1dc(k),vet2
!    pause        
...
!        end if    
    ...
    do i=1,nsujetmeta
       ...
       ! if (i.ge.100) then
        !    print*,i,gmeta(i),nt1meta(i),res2meta(gmeta(i)),dut3(nt1meta(i))
         !   if (i.eq.230) stop
        !endif
...
!        write(*,*)'ig',ig,integrale3(ig)!,nig(ig),cdc(ig),nigmeta(ig)
!        pause
    end do
...
!        if(cpt(k).gt.0.or. cptmeta(k).gt.0)then
        ...
        !if (k.ge.70) then
         !   write(*,*),k,res,res2(k),res2meta(k),res2dc(k),integrale3(k),res1meta(k)
          !  if (k.eq.75) stop
        !endif
        
!             write(*,*)'je suis la 1',dlog(integrale3(k)/((2.d0)*pi*sqrt(theta*eta*(1.d0-alpha**2)))),&
!     ((2.d0)*pi*sqrt(theta*eta*(1.d0-alpha**2))),dlog(integrale3(k))
!    pause
